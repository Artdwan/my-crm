...
export default function LeadsBoard({
  ...
  const [active, setActive] = useState("Лиды");
        ...
        <nav>
          {nav.map((n, i) => (
            <button
              ...
              onClick={() => {
                setActive(n);
                if (n !== "Лиды") note(`Раздел «${n}» будет следующим`);
              }}
            >
              <i>{navIcons[i]}</i>
              {n}
              {n === "Лиды" && <em>{leads.length}</em>}
            </button>
          ))}
        </nav>
      ...
      {selected && (
        ...
        >
          <aside className="drawer" onMouseDown={(e) => e.stopPropagation()}>
            ...
            <button className="primary wide" onClick={() => note("Задача создана")}>
              ＋ Создать задачу
            </button>
          </aside>
        </div>
